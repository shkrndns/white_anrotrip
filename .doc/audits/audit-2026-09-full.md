...
> ## ⚠️ ПЛАН НЕ ЗАВЕРШЁН — пауза с 2026-09-07
>
> Этапы **0–5** выполнены в репозитории; **этап 6 не начинался**; есть хвосты (**1.6 prerender**, деплой VPS).  
> **Статус одной страницей:** [AUDIT-PLAN-STATUS.md](./AUDIT-PLAN-STATUS.md) ← читать перед возобновлением.
...
Также в схему стоит добавить `imageAlt` (сейчас alt для hero не задаётся отдельно) и `draft: z.boolean().default(false)` с фильтрацией в `getCollection` — это уже запланировано в `.doc/blog/max-blog-import-plan.md`.
...
Нужны: PNG 180×180, 192×192, 512×512, `site.webmanifest`, `<meta name="theme-color" content="#00abb3">`. `.doc/roadmaps/pwa-plan.md` описывает полноценный PWA — здесь достаточно первого шага (иконки + manifest без service worker).
...
| `.doc/deploy/notes-blur-production.md:41`                                       | Astro 6.x + Tailwind 4.2.x                              | **7.x + 4.3.x**                     |
| `.doc/_archive/anro-trip-guide-optimized.md:7`                                  | Astro 5.16.13 + Tailwind 4.1.18                         | **7.2.9 / 4.3.2**                   |
| `.doc/_archive/audit-full-2026-04.md:24`, `.doc/roadmaps/project-roadmap.md:10` | Astro 6.x                                               | **7.x**                             |
...
| **Дедуплицировать**                 | `.ai-factory/plans/feature-max-blog-drafts.md` vs `.doc/blog/max-blog-import-plan.md`                                                       | одна тема, два плана                               |
...
`Header.astro` (1233 строки) — план разбиения готов, но требует **явного согласия владельца** (`AGENTS.md`, `.doc/navigation/header-frozen.md`).
...
| 6.4 | Изображения для 6 статей блога + `draft` в схеме | по плану `.doc/blog/max-blog-import-plan.md`                                    |
| 6.5 | Rate-limit в Redis или `caddy-ratelimit`         | когда появится вторая реплика                                                   |
| 6.6 | PWA (service worker, offline)                    | `.doc/roadmaps/pwa-plan.md`; иконки и manifest уже в 2.7                        |
| 6.7 | Яндекс.Метрика                                   | чеклист в `.doc/forms/analytics-cookies-plan.md`; CookieBanner и privacy готовы |
...
### ⏸ ПАУЗА ПЛАНА (2026-09-07)

Работа по аудиту **остановлена намеренно**. Этапы 0–5 в репозитории; **этап 6 не начинался**; хвост **1.6 prerender** и **деплой VPS** — не сделаны.

**Напоминание:** [AUDIT-PLAN-STATUS.md](./AUDIT-PLAN-STATUS.md) — читать перед возобновлением (и для ИИ-агентов).
